...
async function init() {
  ...
  // ⚡ DISPARO PARALELO ASSÍNCRONO (Nenhum barramento de rede bloqueia o outro)
  ...
  // Inicialização independente para a rede remota do Bitcoin (API)
  verificarStatusRedeBitcoin();
  
  // Re-checagem da API do Bitcoin configurada a cada 60 segundos
  ...
  \$("btnMaxUnwrap")?.addEventListener("click", () => {
    const wpol = TOKENS.find(t => t.symbol === "WPOL");
    \(("valorPOL").value = ethers.utils.formatUnits((saldos[wpol.address] \vert{}\vert{} 0n).toString(), 18).replace(/\.?0+\)/, "");
  });
